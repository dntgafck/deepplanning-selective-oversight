...
**Status:** Integrated architecture memo, patched after 2026-04-17 smoke tests; overseer model migrated 2026-04-24  
**Target system:** C2-lite v1.4  
**Changelog (v1.4, 2026-04-24):** Overseer model migrated from DeepSeek-V3.2 to DeepSeek-V4-Flash following DeepSeek's V3.2 deprecation announcement on 2026-04-24. Only §3.1 hard constraints are affected: the overseer model ID, endpoint, and mode labels now reference V4-Flash (Think-High for B/C1/C2; Non-Think for C2-nt). The architecture, hook sites, prompt families, trigger set, and intervention semantics are all model-agnostic and unchanged. See `experimental_protocol_v2_1.md` §1.5 for migration rationale and `thesis_question_v3_1.md` changelog for thesis-level implications.  
...
Archived v1/v2 files and superseded v3 / v2.0 files are historical only and should not drive new design decisions.
...
- Overseer must be **DeepSeek-V4-Flash** (model ID: `deepseek-v4-flash`, direct API at `api.deepseek.com`).
  - thinking mode (Think-High) for B, C1, C2
  - non-thinking mode (Non-Think) for C2-nt
  - Do **not** use the legacy aliases `deepseek-chat` / `deepseek-reasoner`; they are scheduled for retirement after 2026-07-24.
  - Think-Max mode is explicitly **not** used (cost not justified for overseer review tasks).
