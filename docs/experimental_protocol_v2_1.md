...
**Changelog (v2.1, 2026-04-24):** Overseer model migrated from DeepSeek-V3.2 to
DeepSeek-V4-Flash following DeepSeek's V3.2 deprecation announcement on
2026-04-24. V3.2 is scheduled for full retirement after 2026-07-24. V4-Flash
preserves the same family (DeepSeek-AI) and mode split (thinking / non-thinking)
used by the oversight architecture, so no structural change is required — only
model identifier, provider endpoint, and pricing are updated. V3.2 remains cited
in System D baselines (from Zhang et al., 2026, unchanged). Budget estimates
have been recomputed against V4-Flash's lower pricing. See §1.5 for migration
rationale.
...
Qwen family) and overseer (DeepSeek-V4-Flash, DeepSeek-AI) belong to different
model families. This guards against the critique that the overseer is merely a
larger version of the same model. It also ensures that the overseer brings
genuinely different capabilities and failure modes, making any performance gains
...
**Overseer migration note (v2.1).** The overseer was originally specified as
DeepSeek-V3.2. On 2026-04-24, DeepSeek released V4 and announced that V3.2 will
be retired after 2026-07-24. The `deepseek-chat` and `deepseek-reasoner`
endpoints now serve V4-Flash in non-thinking and thinking modes respectively.
V4-Flash is a same-family successor with the same two-mode behavior the
architecture relies on, so the pivot is mechanical: only the model ID and
endpoint change. V4-Flash is also significantly cheaper than V3.2 was (see
§1.4). The V3.2 number in System D (42.5% Case Accuracy, cited from the paper)
is unchanged, since System D is not re-run. The trade-off is a loss of the
tightest form of RQ3's same-model-different-deployment argument: the overseer is
no longer literally the same model as the V3.2 entry in D. The RQ3 claim remains
valid against the other D baselines (GPT-5.2-high, Claude-4.5-Opus-thinking,
Qwen3-Max-thinking), and is strengthened by an optional self-run V4-Flash
monolithic baseline (§1.6) that restores the same-model-different-deployment
comparison for V4-Flash itself.
...
**Overseer cost estimation.** The overseer (DeepSeek-V4-Flash) is priced at
$0.14/M input (cache miss), $0.028/M input (cache hit), and $0.28/M output on
the direct DeepSeek API (`api.deepseek.com`). For comparison, the deprecated
V3.2 via OpenRouter was $0.26/M input and $0.38/M output — V4-Flash is ~46%
cheaper on uncached input and ~26% cheaper on output, with a further ~5×
reduction on cached input. Assuming a ~30% cache hit rate on overseer input
(conservative; system prompt + task query + constraint checklist form a stable
cacheable prefix), the combined overseer cost is approximately half of the
V3.2-on-OpenRouter estimate. Overseer cost per task depends on invocation
frequency and context size. The estimates below assume: System B invokes the
overseer at every interaction turn (~30 turns/task average), C2 invokes at ~20%
of steps (~6 invocations/task), and C1 invokes at ~3–5 checkpoints per task.
Overseer context includes the task query, constraint checklist, and recent
trajectory excerpt (~3K–8K tokens input per call; ~500–2K tokens output per
call). These estimates must be calibrated on the 10-task C2 pilot in Phase 2.
...
**Optional V4-Flash monolithic baseline (to restore clean RQ3).** Running
V4-Flash-thinking as a monolithic single-agent on 120 × 4 Shopping runs costs an
estimated $30–55, depending on thinking-mode output volume. This is a small
addition that restores the same-model-different-deployment comparison for
V4-Flash (C2 uses V4-Flash selectively; baseline uses it monolithically).
Include if budget permits; otherwise rely on the cited D baselines only.

**Budget fit.** The full suite (~$161) is meaningfully closer to the $80–100
target than the v2.0 estimate (~$180) because of V4-Flash's lower pricing. The
protocol retains the prioritized execution plan (Section 6) with explicit
decision points for budget-constrained fallback in case overseer costs exceed
estimates in the pilot.
...
The original v2.0 protocol relied on System D baselines cited from the
DeepPlanning paper and did not re-run any of them. The v2.1 pivot removes the
literal identity between the overseer (previously V3.2) and one specific D entry
(V3.2-thinking, 42.5%). To restore the cleanest form of RQ3 — "the same model
deployed selectively beats it deployed monolithically" — v2.1 introduces an
optional self-run baseline: **DeepSeek-V4-Flash-thinking as a monolithic
single-agent planner**, evaluated on the same 120 Shopping tasks × 4 runs.

**Scope.** Same task loader, same sandbox, same evaluator, same seeds as other
systems. The single agent is given the Shopping task prompt directly and runs
the DeepPlanning tool-calling loop without an overseer or executor split.

**Expected cost.** $30–55 depending on thinking-mode output volume (V4's
Think-High default produces more tokens than V3.2-thinking). Measure on 10 tasks
first to confirm per-task cost before committing to the full run.

**Priority.** Optional but recommended. If budget is tight, this is the first
expansion item after the MVE completes, ranked above System B.

**Interpretation.** If C2 beats V4-Flash monolithic at lower cost, RQ3 is
supported in its strongest form (same model, different deployment). If C2 does
not beat V4-Flash monolithic, the cited D baselines (GPT-5.2, Claude-Opus,
Qwen3-Max) remain available as weaker but still valid comparisons.
...
**Cost computation.** Actual cost is recorded from the provider API response
headers (DeepSeek usage field for overseer; whichever provider serves Qwen for
executor). This is preferred over formula-based estimation because it accounts
for cache-hit discounts and provider-specific pricing. For comparison purposes,
the formula-based estimate is also logged, assuming all input as cache-miss
...
If the provider does not return a cache-hit/miss breakdown, assume all input is
cache-miss (upper bound) and also log the cache-hit rate that would be required
to match the actual reported cost.
...
**Risk level:** Moderate (downgraded from High in v2.0). Empirical executor
costs are ~15× higher than the original pre-experiment estimates, but the
V4-Flash overseer pivot brought the full experimental matrix (~$161) closer to
the $80–100 target. The MVE (~$92) now fits within the original budget.

**Mitigation:** The tiered execution plan (Section 6) ensures the minimum viable
experiment (~$92) answers the core research questions (RQ1, RQ3, and the novel
C2 vs. C2-nt ablation). Track cumulative spend after every batch of 120 runs
using the provider dashboards (DeepSeek Platform for overseer, executor provider
dashboard for executor). The MVE (A + C2 + C2-nt) is prioritized; all other
systems are expansion. If System C2 shows no improvement over A in the pilot
(Phase 2), stop early — the budget is better spent on diagnosis and iteration
than on scaling a non-working architecture.
...
task. If it exceeds $0.08/task on V4-Flash (vs. estimated ~$0.01–0.02), options
include: reducing overseer context window by summarizing trajectory history
instead of passing raw turns; switching to C2-nt as the primary system
(Non-Think mode = fewer output tokens); reducing to 2 runs for all systems; or
switching to V4-Flash Non-Think for runtime triggers while keeping Think-High
only at the final checkpoint. V4-Flash also supports a "Think-Max" mode which is
explicitly not used because its additional cost is not justified for the
overseer review task.

### 7.4 API Rate Limits Slow Experiments

**Risk level:** Low-moderate. Providers (DeepSeek for overseer, executor
provider for executor) may impose per-minute or per-day rate limits.
...
**Note on V4-Flash thinking modes.** V4 introduces three reasoning efforts:
Non-Think, Think-High, and Think-Max. For C2/C1/B the protocol uses Think-High
(the default when `thinking=true`), which is the closest functional analogue to
the V3.2-thinking mode originally specified. Think-Max is not used because its
additional token consumption is not justified by the overseer task profile
(short review of trajectory excerpts). For C2-nt, Non-Think is used.

**Note on cache prefix stability.** To maximise overseer cache hit rate, all
overseer calls within a task share a stable prefix:
`[system role prompt] + [compiled execution contract] + [task query] + [task constraint checklist]`.
Trajectory excerpts and trigger-specific instructions are appended after this
prefix. This is enforced by the overseer prompt builder and should be preserved
across v2.1 and later revisions.
...
- Day 37–40: **Optional V4-Flash monolithic baseline** (§1.6, 120 × 4 runs,
  ~$30–55).
