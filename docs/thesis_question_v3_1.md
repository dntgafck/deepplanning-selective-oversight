...
**Changelog (v3.1, 2026-04-24):** Overseer model updated from DeepSeek-V3.2 to DeepSeek-V4-Flash following DeepSeek's 2026-04-24 release of V4 and the announced retirement of V3.2 after 2026-07-24. The thesis framing is unchanged — selective oversight architecture remains the research object, cross-family executor/overseer pairing is preserved (Qwen + DeepSeek), and the research questions still target reliability, selectivity, competitiveness against strong baselines, complexity sensitivity, and mechanism concentration. The RQ3 wording is updated to reflect that the overseer (V4-Flash) is no longer literally the same model as the V3.2 entry in System D; the same-model-different-deployment argument is preserved for V4-Flash via an optional self-run monolithic baseline (see `experimental_protocol_v2_1.md` §1.6).

## Working thesis question

**Can a selective oversight architecture — pairing a small LLM executor (Qwen3.5-9B) with a selectively triggered stronger overseer (DeepSeek-V4-Flash) — improve end-to-end planning reliability on the DeepPlanning Shopping benchmark, while achieving a better cost–performance tradeoff than both always-on oversight and stronger monolithic single-agent baselines?**
...
Note (v3.1): The original v3 framing relied on DeepSeek-V3.2 being both the overseer and a System D entry (42.5%), giving RQ3 a particularly clean "same model, different deployment" interpretation. After the V3.2 deprecation and migration to DeepSeek-V4-Flash as overseer, the literal same-model comparison is lost: C2 now uses V4-Flash selectively, while D's DeepSeek entry is V3.2 monolithic. The cleanest restoration is the optional self-run V4-Flash-thinking monolithic baseline described in `experimental_protocol_v2_1.md` §1.6. If that baseline is run, RQ3 recovers its strongest form: "does V4-Flash deployed selectively beat V4-Flash deployed monolithically?" If not, RQ3 relies on the other three D baselines (GPT-5.2-high, Claude-4.5-Opus-thinking, Qwen3-Max-thinking), with the V3.2 entry serving as a weaker cross-generation reference.
...
This ablation has not been tested in the literature. DeepSeek-V4-Flash is priced identically in its per-token rates regardless of which thinking mode is selected, but thinking mode (Think-High) produces more output tokens (reasoning traces), increasing cost. If C2-nt performs comparably to C2, it implies that lightweight, fast oversight is viable — a finding with practical implications for production deployment.
...
C2 can match or exceed the Shopping Case Accuracy of at least one System D baseline at lower total system cost. The primary target is the weakest D baseline (DeepSeek-V3.2-thinking: 42.5%); stretch targets are Qwen3-Max-thinking (43.5%) and Claude-4.5-Opus-thinking (45.0%); GPT-5.2-high (54.2%) is unlikely to be matched. If the optional V4-Flash monolithic self-run baseline is included (protocol §1.6), matching or beating that baseline at lower cost is the strongest form of this hypothesis.
...
Qwen3.5-9B executor with DeepSeek-V4-Flash (thinking) reviewing every step. Establishes the oversight ceiling: maximum accuracy gain at maximum cost. Allocated 2 runs (vs. 4 for core systems) due to its high per-run cost (~$11/run).

## System C1 — Small executor + checkpoint-only oversight (budget-contingent)

Qwen3.5-9B executor with DeepSeek-V4-Flash (thinking) reviewing only at natural checkpoints (per-item and final cart). Tests whether mid-trajectory intervention adds value over periodic review.

## System C2 — Small executor + adaptive selective oversight (Primary)

Qwen3.5-9B executor with DeepSeek-V4-Flash (thinking) triggered by the adaptive filter. The primary thesis system. All five trigger types active. Graduated correction protocol.
...
Actual API cost reported by the provider (DeepSeek for overseer, executor's provider for executor), covering executor tokens + overseer tokens. This is the denominator of the cost–performance tradeoff.
