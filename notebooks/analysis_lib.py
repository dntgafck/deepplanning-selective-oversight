...
SYSTEM_META = {
    ...
    "C2":          {"executor": "qwen3.5-9b",        "overseer": "deepseek-v4-flash", "kind": "selective"},
    "C2-nt":       {"executor": "qwen3.5-9b",        "overseer": "deepseek-v4-flash", "kind": "selective-no-think"},
    "C2-noretry":  {"executor": "qwen3.5-9b",        "overseer": "deepseek-v4-flash", "kind": "selective-no-retry"},
    "C2-deepseek": {"executor": "deepseek-v4-flash", "overseer": "deepseek-v4-flash", "kind": "selective-ds"},
...
def load_aggregated(archives_root: Path) -> pd.DataFrame:
    ...
    for archive_name, system in ARCHIVE_TO_SYSTEM.items():
        ...
        if not archive_dir.exists():
            continue
        session_root = _find_session_root(archive_dir)
        for agg in sorted((session_root / "aggregated_results").glob("*_aggregated.json")):
            with agg.open() as f:
                data = json.load(f)
...
def load_langfuse(csv_path: Path) -> pd.DataFrame:
    ...
    name_to_system = {v_name: v_sys for v_name, v_sys in zip(
        ["shopping-a", "shopping-c2", "shopping-c2-deepseek",
         "shopping-c2-noretry", "shopping-c2-nt", "shopping-d"],
    ...
    for _, r in raw.iterrows():
        ...
        for u in usage:
            ...
            if (executor_model and executor_model.lower() in model
                    and overseer_model and overseer_model.lower() in model
...
def system_token_summary(lf: pd.DataFrame) -> pd.DataFrame:
    ...
    g["cache_hit_rate"] = np.where(
        g["input_total"] > 0, g["input_cached"] / g["input_total"], np.nan,
    )
...
def cost_dollars(lf: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    sys_tok = system_token_summary(lf)
    ...
    for system, grp in lf.groupby("system"):
        ...
        for _, r in grp.iterrows():
            ...
            if "deepseek" in m:
                ...
                uncached_usd += (
                    (r["input_uncached"] + r["input_cached"]) / 1e6 * PRICE_DS_INPUT_UNCACHED
                  + r["output"] / 1e6 * PRICE_DS_OUTPUT
                )
            elif "qwen" in m:
                qwen_cost = (
...
def per_run_per_level(per_case: pd.DataFrame) -> pd.DataFrame:
    ...
    g = (per_case
         .groupby(["system", "run", "level"], as_index=False)
         .agg(case_accuracy=("case_score", "mean"),
...
def per_run_overall(per_case: pd.DataFrame) -> pd.DataFrame:
    ...
    expected = (per_case.groupby(["system", "run"])["expected_count"].sum()
...
def system_summary(per_case: pd.DataFrame) -> pd.DataFrame:
    ...
    overall_summary = (overall.groupby("system", as_index=False)
                              .agg(case_acc_mean=("case_accuracy", "mean"),
                                   case_acc_std =("case_accuracy", "std"),
                                   match_rate_mean=("match_rate", "mean"),
                                   runs=("case_accuracy", "size")))

    per_level = per_run_per_level(per_case)
    level_pivot = (per_level.groupby(["system", "level"], as_index=False)
                            .agg(case_acc_mean=("case_accuracy", "mean"),
                                 case_acc_std =("case_accuracy", "std")))
    level_wide_mean = level_pivot.pivot(index="system", columns="level",
                                        values="case_acc_mean")
    level_wide_mean.columns = [f"L{c}_mean" for c in level_wide_mean.columns]
    level_wide_std  = level_pivot.pivot(index="system", columns="level",
                                        values="case_acc_std")
    level_wide_std.columns = [f"L{c}_std" for c in level_wide_std.columns]
    out = (overall_summary
           .merge(level_wide_mean, left_on="system", right_index=True)
...
def head_to_head(per_case: pd.DataFrame, sys_a: str, sys_b: str) -> pd.DataFrame:
    ...
    pa = (per_case[per_case.system == sys_a]
          .groupby(["level", "case_name"], as_index=False)["case_score"].mean()
